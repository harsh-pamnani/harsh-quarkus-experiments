...
class SomeServiceTest {
    ...
    // Here `someRepo.update()` is called, but its internal Uni is not executed.
    // So, `HPXYZ: Update uni executed` will NOT be logged
    // Test case fails because we have verification that the Uni is subscribed at least once. Which is not happening. So, it fails.
    @Disabled
    @Test
    void ackMobileEvent_take2() {
        ...
        UniCreateFromItemSupplier<String> updatedUni = spy(new UniCreateFromItemSupplier<>(() -> "Mobile event from DB"));
    ...
    // Here `someRepo.update()` is called, and its internal Uni is also executed because `fixedAckMobileEvent` uses `transformToUni`.
