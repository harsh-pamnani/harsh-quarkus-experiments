...
public class MessageStatusChecker {
    ...
    public void scheduleMessageForRetry(String sessionId, Integer offset) {
        retryDataMap.put(
                new EventData(sessionId, offset), 0); // 0 represents the count for number of retries. Starting with 0
    }
    ...
    void checkMessageStatus() {
        ...
        retryDataMap.forEach((data, count) -> {
            if (count >= MAXIMUM_RETRIES) {
                log.info("MAXIMUM retries reached. Cancelling the message. sessionId: " + data.sessionId()
                        + ", offset: " + data.offset());
            ...
            if (isAcknowledged(data.sessionId(), data.offset())) {
                log.info("Message IS acknowledged, removing it from retry data. sessionId: " + data.sessionId()
                        + ", offset: " + data.offset());
                retryDataMap.remove(data);
            } else {
                log.info("Message IS NOT acknowledged, increasing retry count. sessionId: " + data.sessionId()
                        + ", offset: " + data.offset());
